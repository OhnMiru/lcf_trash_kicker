...
# Создаём event loop для основного потока ДО импорта
try:
...
async def process_api_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("**Шаг 2 из 5:** Введите API HASH", parse_mode="Markdown")
...
async def process_api_hash(message: types.Message, state: FSMContext):
    ...
    await message.answer("**Шаг 3 из 5:** Номер телефона с +\nПример: `+79001234567`", parse_mode="Markdown")
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("✅ **Код отправлен!**\n\n**Шаг 4 из 5:** Введите код", parse_mode="Markdown")
...
async def process_code(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if hasattr(result, 'is_password_required') and result.is_password_required:
            await message.answer("**Шаг 5 из 5:** Введите пароль 2FA", parse_mode="Markdown")
        ...
        await message.answer("✅ **Авторизация завершена!**\n\nТеперь выполните /setup", parse_mode="Markdown")
        await state.clear()
...
async def setup_command(message: types.Message):
    ...
    if len(args) != 3:
        await message.answer("❌ Использование: /setup ID_канала ID_группы\nПример: `/setup -1001234567890 -1009876543210`", parse_mode="Markdown")
        return
...
async def process_exclude_list(message: types.Message):
    ...
    confirm_text = f"⚠️ **Подтверждение**\n\n📌 Пост: {active_data['post_link']}\n👥 Не отметилось: {len(new_ids)}"
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да", callback_data=f"confirm_yes_{active_temp_id}"),
         InlineKeyboardButton(text="✏️ Ещё", callback_data=f"edit_{active_temp_id}"),
...
async def handle_confirm_yes(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"✅ **Готово!**\n\n📌 Пост: {data['post_link']}\n📊 Не отметилось: {data['total_count']}\n🚫 Удалено: {result['success']}\n⚠️ Ошибок: {result['errors']}",
        parse_mode="Markdown"
    )
    del pending_cleanups[temp_id]
...
async def process_post(post_id: int, deadline: float, reply_chat_id: int, post_link: str, user_id: int):
    ...
    confirm_text = f"⚠️ **Требуется подтверждение**\n\n📌 Пост: {post_link}\n👥 Не отметилось: {len(to_kick)}\n\n**Список:**\n{list_text}\n\n❗️ Удалить из канала?"
...
if __name__ == "__main__":
    ...
    flask_thread.daemon = True  # Поток завершится при остановке основного
    flask_thread.start()
    
    # Запускаем бота в основном потоке (важно для сигналов)
